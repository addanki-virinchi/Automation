...
def build_search_page_url(keyword: str, page_num: int) -> str:
    slug = slugify(keyword)
    base_url = SEARCH_BASE_TEMPLATE.format(slug=slug)
    if page_num > 1:
        base_url = f"{base_url}-{page_num}"
    encoded_keyword = quote(keyword.strip())
    query = SEARCH_QUERY.format(keyword=encoded_keyword)
    return f"{base_url}?{query}"


def navigate_to_search_page(driver, keyword: str, page_num: int) -> bool:
    url = build_search_page_url(keyword, page_num)
    logging.info("Navigating to search page %s for %s: %s", page_num, keyword, url)
    try:
        driver.get(url)
    ...
    time.sleep(1)
    if not wait_for_job_cards(driver, timeout=15):
        logging.warning("No job cards detected on search page %s.", page_num)
...
def is_stale_element(element) -> bool:
    try:
        element.is_enabled()
        return False
...
def main():
    ...
    global DEBUG_DUMP_HTML, MAX_PAGES, MAX_JOBS_PER_PAGE, SEARCH_START_PAGE
    ...
    try:
        ...
        with open(OUTPUT_CSV, "a", newline="", encoding="utf-8") as f:
            ...
            logging.info(
                "Search pagination enabled. Pages %s-%s for each keyword.",
                SEARCH_START_PAGE,
            ...
            for keyword in SEARCH_KEYWORDS:
                ...
                for page_number in range(SEARCH_START_PAGE, SEARCH_START_PAGE + MAX_PAGES):
                    if not navigate_to_search_page(driver, keyword, page_number):
                        break
                    logging.info(
                        "Processing page %s for %s at %s",
                        page_number,
                        keyword,
                        driver.current_url,
                    )
